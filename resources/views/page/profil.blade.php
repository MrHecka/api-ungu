...
<div class="flex h-screen w-full items-center justify-center bg-gray-900 bg-cover bg-no-repeat" style="background-image:url({{url('img/bgAuth.webp')}})">
  <div class="rounded-xl bg-gray-800 bg-opacity-50 px-16 py-10 shadow-lg backdrop-blur-md max-sm:px-8">
    ...
    <form action="{{ route('profile.update') }}" method="POST">
        ...
        <div class="mt-8 flex justify-content-center text-lg text-white-900 font-bold">
          <button type="submit" class="rounded-3xl bg-purple-400 bg-opacity-50 px-10 py-2 text-white-900 dark:text-white shadow-xl backdrop-blur-md transition-colors duration-300 hover:bg-purple-600">Edit</button>
          <a href="{{ route('register') }}" class="ml-4 text-blue-500 hover:underline font-bold hover:font-semibold">Ganti Password</a>
        </div>
